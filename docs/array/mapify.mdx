...
- Often called `arrayToMap`, `collectionToMap`, or `arrayMap` in other libraries.

### Popular use cases

- Transforming an array of objects into a `Map` using unique keys, such as converting user data into a lookup structure where user IDs map to user names.
- Creating a frequency count of items in an array by mapping each item to its occurrence count.
- Converting complex structures like nested arrays or objects into a flat key-value structure suitable for quick retrieval and manipulation.
