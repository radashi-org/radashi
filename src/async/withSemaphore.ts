 ...
 * ```
 */
export function withSemaphore<T>(
  capacity: number,
  fn: AnyFn<T>,
): PrebuiltExclusiveFn<T>
...
export function withSemaphore(
  optionsOrCapacity: number | WithSemaphoreOptions,
  baseFn?: AnyFn,
): PrebuiltExclusiveFn<unknown> {
  const options = useOptions(optionsOrCapacity)
  if (options.capacity < 1) {
    throw new Error(`invalid capacity ${options.capacity}: must be positive`)
  }
  ...
  function acquire(weight = 1) {
    ...
    if (weight > options.capacity) {
      throw new Error(
        `invalid weight ${weight}: must be lower than or equal capacity ${options.capacity}`,
      )
    }
  ...
  async function runExclusive(
    optionsOrWeightOrFn?: number | ExclusiveOptions | AnyFn,
    innerFn?: AnyFn,
  ): Promise<unknown> {
    const options =
      typeof optionsOrWeightOrFn === 'function'
        ? {}
        : useExclusiveFnOptions(optionsOrWeightOrFn)
    const fn =
      (typeof optionsOrWeightOrFn === 'function'
        ? optionsOrWeightOrFn
        : innerFn) ?? baseFn
    if (!fn) {
      throw new Error('invalid execution: function is required')
    }
...
function useExclusiveFnOptions(
  optionsOrWeight?: number | ExclusiveOptions,
): ExclusiveOptions {
